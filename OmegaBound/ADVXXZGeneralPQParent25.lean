...
theorem parent25_projected_beta_count {w s b : ℕ}
    (p : ConstituentInput w s) (d : ConstituentSpec p) (ε : ℚ) (m : ℕ)
    (r : Fin 6) (W : Fin 2) (β : RepresentedLaw p d b ε m r W)
...
/-- The complete paired P-law has the exact parentwise projection fibres of
`Parent25.PProjectionBridge`. -/
theorem constituent_P_projection_fibres : Parent25.PProjectionBridge := by
  intro w s b p d _hd _hb ε m r W β J
  constructor
  · exact ⟨parent25PFiberEquiv p d ε m r W β J⟩
  · intro t c
    exact parent25_projected_beta_count p d ε m r W β J t c
...
theorem parent25_joint_cardinality_identities {w s b : ℕ}
    (p : ConstituentInput w s) (d : ConstituentSpec p) (ε : ℚ) (m : ℕ)
    (r : Fin 6) (W : Fin 2) (β : RepresentedLaw p d b ε m r W) :
