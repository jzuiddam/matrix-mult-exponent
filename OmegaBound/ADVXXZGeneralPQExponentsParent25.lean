import OmegaBound.ADVXXZGeneralQTypeClassesParent25
...
private theorem apq_alphaCount_cast {w s b : ℕ}
  ...
  rcases (hb.2 t r).2 v |>.1 with ⟨n, hn⟩
  have hscaled :
...
private theorem apq_alphaCount_cast_real {w s b : ℕ}
  ...
  exact_mod_cast apq_alphaCount_cast p d hb m r t v
...
private theorem apq_projected_normalized {w s b : ℕ}
  ...
  have hscaleq := apq_parentCount_cast p d hb m r t
  have hscaleq_ne :
  ...
  rw [Nat.cast_sum]
  simp_rw [apq_alphaCount_cast_real p d hb m r t]
...
private theorem apq_entropyNats_eq_log_two_mul_entropy
...
theorem entropyNats_eq_log_two_mul_entropy
    {alpha : Type*} [Fintype alpha] (f : alpha → ℝ) :
    entropyNats f = Real.log 2 * entropy f :=
  apq_entropyNats_eq_log_two_mul_entropy f
...
private theorem apq_childCount_cast {w s b : ℕ}
    (p : ConstituentInput w s) (d : ConstituentSpec p)
    (hb : StepIntegralAt p d b) (m : ℕ) (r : Fin 6) (W : Fin 2)
    (t : Fin s) (v : ChildShape p t) (σ : Chunk w) :
    (Parent25.childCount p d m r W t v σ : ℚ) =
      (m : ℚ) * d.outBase ⟨t, r, v⟩ *
        (d.betaChild (Parent25.side d r W) t r v).prob σ := by
  rcases ((hb.2 t r).2 v).2.2 (Parent25.side d r W) σ with ⟨n, hn⟩
  have hscaled :
      (m : ℚ) * d.outBase ⟨t, r, v⟩ *
          (d.betaChild (Parent25.side d r W) t r v).prob σ =
        ((m * n : ℕ) : ℚ) := by
    calc
      _ = (m : ℚ) *
          ((d.outBase ⟨t, r, v⟩ : ℚ) *
            (d.betaChild (Parent25.side d r W) t r v).prob σ) := by ring
      _ = (m : ℚ) * n := by rw [hn]
      _ = ((m * n : ℕ) : ℚ) := by norm_cast
...
private theorem apq_residual_histogram_cast {w s b : ℕ}
    (p : ConstituentInput w s) (d : ConstituentSpec p)
    (hd : ConstituentAdmissibleAt d b) (hb : StepIntegralAt p d b)
    (m : ℕ) (r : Fin 6) (W : Fin 2) (t : Fin s) (k : Fin (2*w+1))
    (σ : Chunk w) :
  ...
  · simp only [hu, if_pos]
    simpa only [mul_assoc] using apq_childCount_cast_real p d hd hb m r W t u σ
  · simp [hu]

private theorem apq_boundary_cell_entropy_of {w s b : ℕ}
    (p : ConstituentInput w s) (d : ConstituentSpec p)
    (hd : ConstituentAdmissibleAt d b) (hb : StepIntegralAt p d b)
    (m : ℕ) (r : Fin 6) (W : Fin 2) (J : AlphaLabel p d b m r)
  ...
  have htotal := (constituent_Q_type_classes p d hd hb m r W J).2 t (.inl v)
  have hhist : ∀ σ : Chunk w,
    ...
    exact apq_childCount_cast_real p d hd hb m r W t v σ
...
private theorem apq_residual_cell_entropy {w s b : ℕ}
  ...
  have hhist : ∀ σ, (Parent25.cellHistogram p d m r W t (.inr k) σ : ℝ) =
      L * num σ := by
    intro σ
    exact apq_residual_histogram_cast p d hd hb m r W t k σ
  have htotal := (constituent_Q_type_classes p d hd hb m r W J).2 t (.inr k)
...
private theorem apq_q_parent_entropy {w s b : ℕ}
  ...
  have hcells :
    ...
    · apply Finset.sum_congr rfl
      ...
      · rw [if_pos hv]
        exact apq_boundary_cell_entropy_of p d hd hb m r W J t v hv
      · rw [if_neg hv]
        exact apq_boundary_cell_entropy_not p d hd hb m r W J t v hv
    · apply Finset.sum_congr rfl
      intro k _
      exact apq_residual_cell_entropy p d hd hb m r W J t k
  rw [hcells]
  by_cases hW : W = 0
  · subst W
    simp only [Parent25.boundary, Parent25.side, if_pos]
    simp_rw [apq_entropyNats_eq_log_two_mul_entropy]
