...
theorem parent25_partition_bridge : Parent25.PartitionBridge := by
  intro w s b p d _hd _hb m r W j a
  constructor
...
private theorem parent25_childCount_cast {w s b : ℕ}
  ...
  rcases ((hb.2 t r).2 v).2.2 (Parent25.side d r W) sigma with ⟨n, hn⟩
  have hscaled :
      (m : ℚ) * d.outBase ⟨t, r, v⟩ *
          (d.betaChild (Parent25.side d r W) t r v).prob sigma =
        ((m * n : ℕ) : ℚ) := by
    calc
      _ = (m : ℚ) *
          ((d.outBase ⟨t, r, v⟩ : ℚ) *
            (d.betaChild (Parent25.side d r W) t r v).prob sigma) := by ring
      _ = (m : ℚ) * n := by rw [hn]
      _ = ((m * n : ℕ) : ℚ) := by norm_cast
...
private theorem parent25_sum_childCount {w s b : ℕ}
  ...
  rw [Nat.cast_sum]
  simp_rw [parent25_childCount_cast p d hb m r W t v]
...
private theorem parent25_childTotal_eq_alphaPair {w s b : ℕ}
    ...
    (m : ℕ) (r : Fin 6) (t : Fin s) (v : ChildShape p t) :
    m * d.outBase ⟨t, r, v⟩ =
      Parent25.alphaCount p d b m r t v +
        Parent25.alphaCount p d b m r t (complement p t v) := by
  apply Nat.cast_injective (R := ℚ)
  push_cast
  rw [parent25_alphaCount_cast p d hb m r t v,
    parent25_alphaCount_cast p d hb m r t (complement p t v), hd.out_eq t r v]
  push_cast
  ring
...
private theorem parent25_sum_cellHistogram {w s b : ℕ}
  ...
  · simp only [hv, if_true]
    rw [parent25_sum_childCount p d hb m r W t v,
      parent25_childTotal_eq_alphaPair p d hd hb m r t v]
  · simp [hv]
...
private noncomputable def parent25_qFiberEquiv {w s b : ℕ}
  ...
  let F : Parent25.QFiber p d b m r W j.val →
    ...
    E t ⟨(fun ih => x.val ⟨t, ih⟩),
      (parent25_partition_bridge p d hd hb m r W j x.val).mp x.property.2 t⟩
  ...
  · intro X
    ...
    let q : Parent25.QFiber p d b m r W j.val :=
      ⟨a, parent25_histograms_contains p d hd m r W j a hhist,
        (parent25_partition_bridge p d hd hb m r W j a).mpr hhist⟩
    refine ⟨q, ?_⟩
    funext t
...
theorem constituent_Q_type_classes : Parent25.QTypeClassBridge := by
  intro w s b p d hd hb m r W j
  constructor
  · exact ⟨parent25_qFiberEquiv p d hd hb m r W j⟩
  · intro t c
    exact parent25_sum_cellHistogram p d hd hb m r W j t c
